...
# Set by cleanup() so the main path can tell an intentional (signal) stop from a crash - only the former exits 0.
SHUTDOWN_REQUESTED=0

# Budget here (~5s: TERM then KILL) must stay within systemd's TimeoutStopSec=15 for this unit.
cleanup() {
...
# Capture the child's exit status without tripping `set -e` (wait on the RHS of `||` is exempt).
# A signal-stop exits 0; any other non-zero is a crash that must propagate so systemd triggers Restart.
status=0
